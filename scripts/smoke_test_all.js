...
(async ()=>{
  ...
  async function checkLang(code){
    ...
    return { code, dir, lang, secExists: !!sec, secOk, selectExists: !!select, selectOk, selectValue: select ? select.value : null };
  }
  ...
  for(const code of codes){
    ...
    results.push(res);
    if(!res.secOk || !res.selectOk){
      console.log('FAIL:', JSON.stringify(res));
    }
  }

  const fails = results.filter(r=>!(r.secOk && r.selectOk) || r.error);
